...
fn u16_test() {
    ...
    let mut var = default_data_provider().variable_for(&spec);
    let int = var.integer_mut();
    int.set_u64(5001u64).unwrap();
    assert_eq!(int.u64().unwrap(), 5001u64);
}
...
fn i16_test() {
    ...
    let mut var = default_data_provider().variable_for(&spec);
    let int = var.integer_mut();
    int.set_i64(-5001i64).unwrap();
    assert_eq!(int.i64().unwrap(), -5001i64);
}
...
fn u8_overflow_test() {
    let spec = IntegerSpecBuilder::new()
        ...
        .set_storage(IntegerStorage::B8)
        .build();
    let in_value: u64 = 300u64;
    let mut var = default_data_provider().variable_for(&spec);
    let int = var.integer_mut();
    assert_eq!(
        int.set_u64(in_value).unwrap_err(),
...
fn u32_overflow_test() {
    let spec = IntegerSpecBuilder::new()
        ...
        .set_storage(IntegerStorage::B32)
        .build();
    let in_value: u64 = 70000000000u64;
    let mut var = default_data_provider().variable_for(&spec);
    let int = var.integer_mut();
    assert_eq!(
        int.set_u64(in_value).unwrap_err(),
